...
"kg/L" · "g/L" · "mL/L" · "L/L" (actualizado — antes se aceptaba en minúsculas, ver "Cambios recientes" al final del documento)

## Cambios recientes — Nomenclatura de `dosis_unidad` (2026-07-08)

`dosis_unidad` cambió de notación en minúsculas a notación estándar con "L" (litro) en mayúscula (comparte enum con `Quimico.rate_unidad`).
...
- Los registros existentes se migraron automáticamente — no requiere backfill del front.
...
El módulo de Recetas se eliminó por completo. El campo `receta_id` mencionado en varias secciones de este documento (body de creación, query params de listado, `RECETA_NOT_FOUND`, `APLICACION_CONTEXTO_INVALIDO`) **ya no existe** — el backend lo había quitado de `aplicaciones_quimicas` en un refactor previo y ahora se eliminó también el catálogo de recetas. Ignorar cualquier referencia a `receta_id` en las secciones anteriores de este documento.

**Nota aparte:** partes de las secciones 8–11 de este documento (modelo de datos, contexto `"invernadero"` vs el valor real `"greenhouse"` que usa la API, forma de `detalles`) quedaron desactualizadas por refactors anteriores no reflejados acá. Si necesitás integrar contra este módulo, pedí una regeneración completa del documento antes de guiarte por las secciones 8–11.